    ...
    @inbounds if node > 0 && depth < max_depth && (is_mle2p ? best_gain[n_idx] > 0 : best_gain[n_idx] > gamma)
        ...
        elseif is_mle2p
            ...
            if w > epsv
                ...
                g2 = nodes_sum[2, node]
                h2 = nodes_sum[4, node]
                tree_pred[1, node] = (-g1 / (h1 + lambda * w + L2 * eltype(tree_pred)(1e-4) + epsv)) / bagging_size
                tree_pred[2, node] = (-g2 / (h2 + lambda * w + L2 * eltype(tree_pred)(1e-4) + epsv)) / bagging_size
            end
        else
            w = nodes_sum[2*K+1, node]
            if w > epsv
    ...
    @inbounds if node > 0
        ...
        if is_mle2p && K == 2
            ...
            if w > eps
                ...
                gain = (g1^2 / max(eps, (h1 + lambda * w + L2 * T(1e-4))) + g2^2 / max(eps, (h2 + lambda * w + L2 * T(1e-4)))) / 2
            end
        else
