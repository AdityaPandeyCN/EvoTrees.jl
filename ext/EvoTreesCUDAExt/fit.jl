...
@kernel function sum_grads_kernel!(nodes_sum, @Const(∇), @Const(is))
    ...
    for i_obs in tid_global:total_threads:length(is)
        @inbounds if i_obs <= length(is)  # Added bounds check
            idx = is[i_obs]
            g1 += ∇[1, idx]
            g2 += ∇[2, idx]
        end
    ...
    # Fixed: Improved reduction with proper bounds checking
    stride = 128
    while stride >= 1
        @synchronize()
        ...
        stride = stride >> 1  # More efficient than >>>= 
    end
    @synchronize()
...
@kernel function apply_splits_kernel!(
    ...
    nodes_sum, nodes_gain, h∇,  # Added h∇ to get children stats
    ...
    @inbounds if n_idx <= length(active_nodes)
        ...
        if depth < max_depth && best_gain[n_idx] > nodes_gain[node] + gamma && best_bin[n_idx] > 0
            ...
            # Get feature and bin for this split
            feat = Int(tree_feat[node])
            bin = Int(tree_cond_bin[node])
            
            # Calculate children statistics from histogram
            # Left child gets sum up to bin, right child gets remainder
            l_g1 = zero(eltype(nodes_sum))
            l_g2 = zero(eltype(nodes_sum))
            l_w = zero(eltype(nodes_sum))
            
            # Sum up to the split bin for left child
            for b in 1:bin
            ...
            # Calculate children gains
            nodes_gain[child_l] = l_g1^2 / (l_g2 + lambda + eltype(nodes_gain)(1e-8))
            nodes_gain[child_r] = r_g1^2 / (r_g2 + lambda + eltype(nodes_gain)(1e-8))
            ...
            # Leaf node - calculate prediction
            g = nodes_sum[1, node]
            h = nodes_sum[2, node]
...
@kernel function get_gain_kernel!(nodes_gain, @Const(nodes_sum), @Const(nodes), lambda::T) where T
    n_idx = @index(Global)
...
function grow_tree!(tree::EvoTrees.Tree{L,K}, params::EvoTrees.EvoTypes{L}, cache, edges, is, js) where {L,K}
    ...
    # Fixed: Proper kernel instantiation
    ...
    # Fixed: Proper kernel instantiation for get_gain
    kernel_get_gain! = get_gain_kernel!(backend)
    kernel_get_gain!(
        cache.nodes_gain_gpu, cache.nodes_sum_gpu, 
        CuArray(Int32[1]), eltype(cache.nodes_gain_gpu)(params.lambda); 
        ndrange=1
    )
    KernelAbstractions.synchronize(backend)
    ...
    for depth in 1:params.max_depth
        ...
        # Fixed: Proper kernel instantiation for apply_splits
        ...
        kernel_apply_splits!(
            ...
            cache.nodes_sum_gpu, cache.nodes_gain_gpu, cache.h∇,  # Added h∇
            cache.n_next_gpu, cache.n_next_active_gpu,
            view_gain, view_bin, view_feat, active_nodes,
            depth, params.max_depth, 
            eltype(cache.tree_gain_gpu)(params.lambda), 
            eltype(cache.tree_gain_gpu)(params.gamma);
            ndrange = n_active
        )
        KernelAbstractions.synchronize(backend)
...
function grow_otree!(tree::EvoTrees.Tree{L,K}, params::EvoTrees.EvoTypes{L}, cache, edges, is, js) where {L,K}
    ...
    # TODO: Implement optimized GPU version for oblivious trees
    return nothing
end
