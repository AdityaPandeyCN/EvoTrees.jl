                    ...
                    for b in 1:(nbins - 1)
                        ...
                        if s3 >= min_weight && (w_p - s3) >= min_weight
                            ...
                            if constraint != 0
                                ...
                                predR = zero(T)
                                if loss_group == 3  # Credibility
                                    predL = l_g1 / (s3 + L2 + T(1e-8))
                                    predR = r_g1 / ((w_p - s3) + L2 + T(1e-8))
                                elseif loss_group == 4  # MAE
                                    predL = l_g1 / ((1 + lambda) * s3 + L2 + T(1e-8))
                                    predR = r_g1 / ((1 + lambda) * (w_p - s3) + L2 + T(1e-8))
                                else  # Standard
                    ...
                    for b in 1:(nbins - 1)
                        ...
                        if l_w >= min_weight && r_w >= min_weight
                            if constraint != 0
                                ...
                                predR = zero(T)
                                if loss_group == 3  # Credibility
                                    predL = l_g1 / (l_w + L2 + T(1e-8))
                                    predR = r_g1 / (r_w + L2 + T(1e-8))
                                elseif loss_group == 4  # MAE
                                    predL = l_g1 / ((1 + lambda) * l_w + L2 + T(1e-8))
                                    predR = r_g1 / ((1 + lambda) * r_w + L2 + T(1e-8))
                                else  # Standard
...
function update_hist_gpu!(
    ...
    left_nodes_buf, right_nodes_buf, target_mask_buf, feattypes, monotone_constraints, K, ::Type{L}
) where {L}
