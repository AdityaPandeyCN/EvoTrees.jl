...
# Update observation-to-node assignments by traversing splits
...
# Build gradient histograms using atomic operations
...
# Zero histogram entries for active nodes
...
# Split siblings into build (smaller) vs subtract (larger) lists
...
# Compute histogram via subtraction: child = parent - sibling
...
# Accumulate gradient sums for root node
...
# Sum histogram across bins to get node gradient totals
...
# Find best split for each (node, feature) pair
...
# Reduce per-feature gains to find best split per node
