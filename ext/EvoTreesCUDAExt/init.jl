function EvoTrees.init_core(params::EvoTrees.EvoTypes, ::Type{<:EvoTrees.GPU}, data, fnames, y_train, w, offset)
    ...
    x_bin = CuArray(EvoTrees.binarize(data; feature_names=fnames, edges=edges))
    ...
    h∇ = KernelAbstractions.zeros(backend, Float32, 2 * K + 1, Int(maximum(featbins)), Int(length(featbins)), Int(2^params.max_depth - 1))
    h∇L = KernelAbstractions.zeros(backend, Float32, 2 * K + 1, Int(maximum(featbins)), Int(length(featbins)), Int(2^params.max_depth - 1))
    h∇R = KernelAbstractions.zeros(backend, Float32, 2 * K + 1, Int(maximum(featbins)), Int(length(featbins)), Int(2^params.max_depth - 1))
    ...
    js = KernelAbstractions.zeros(backend, UInt32, Int(ceil(params.colsample * nfeats)))
